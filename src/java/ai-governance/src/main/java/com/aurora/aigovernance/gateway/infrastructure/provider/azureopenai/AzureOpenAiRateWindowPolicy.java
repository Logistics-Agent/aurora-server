...
@Component
public class AzureOpenAiRateWindowPolicy implements ProviderRateWindowPolicy {
    ...
    public RateWindow rpmWindow(ProviderSlot slot, Instant now) {
        return new RateWindow("rpm:" + MINUTE_FMT.format(now), 70);
    }
    ...
    public RateWindow rpdWindow(ProviderSlot slot, Instant now) {
        return new RateWindow("rpd:" + DAY_FMT.format(now), 90000);
    }
}
