...
public interface PrApprovalRecordJpaRepository extends JpaRepository<PrApprovalRecord, UUID> {
    ...
    Optional<PrApprovalRecord> findByIncidentIdAndStage(UUID incidentId, ApprovalStage stage);
    List<PrApprovalRecord> findByStatus(ApprovalStatus status);
    List<PrApprovalRecord> findByStageAndStatus(ApprovalStage stage, ApprovalStatus status);
    List<PrApprovalRecord> findByStatusAndExpiresAtBefore(ApprovalStatus status, Instant expiryTime);
}
